...
func (f FieldsV1) Equal(f2 FieldsV1) bool {
	// Fast path: if they point to the exact same interned memory, they are equal.
	if f.handle == f2.handle {
		return true
	}
	// Fallback: If one is uninitialized (zero-value handle) and the other is an explicitly empty handle (unique.Make("")),
	// the handles won't match, but GetRawString() safely evaluates both to "" and ensures they are considered equal
	// to maintain parity with legacy bytes.Equal(nil, []byte{}) behavior. If these were treated as unequal, it would
	// trigger false-positive diffs during managedFields merging in the API server, causing redundant object updates.
