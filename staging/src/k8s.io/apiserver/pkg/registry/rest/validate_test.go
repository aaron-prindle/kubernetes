	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			featuregatetesting.SetFeatureGateDuringTest(
				t, utilfeature.DefaultFeatureGate, features.DeclarativeValidationTakeover, tt.takeoverEnabled,
			)

			wrapped := WithRecover(tt.validateFn)
...
func TestWithRecoverUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wrapped := WithRecoverUpdate(tt.validateFn)
