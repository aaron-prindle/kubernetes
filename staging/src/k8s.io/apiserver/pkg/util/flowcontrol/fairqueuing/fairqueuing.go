...
func initQueues(numQueues int) []*Queue {
	...
	for i := 0; i < numQueues; i++ {
		fqqueues[i] = &Queue{Index: i, Packets: make([]*Packet, 0)}
...
func (qs *QueueSetImpl) TimeoutOldRequestsAndRejectOrEnqueue(hashValue uint64, handSize int32) *Packet {
...
func (qs *QueueSetImpl) removeTimedOutPacketsFromQueue(queue *Queue) {
	timeoutIdx := -1
	now := qs.clk.Now()
	pkts := queue.Packets
...
func (qs *QueueSetImpl) ChooseQueueIdx(hashValue uint64, handSize int) int {
	...
	return shuffleDealAndPick(hashValue, uint64(qs.desiredNumQueues),
		func(idx int) int { return len(qs.queues[idx].Packets) },
...
func (qs *QueueSetImpl) rejectOrEnqueue(packet *Packet) bool {
	queue := packet.Queue
	curQueueLength := len(queue.Packets)
...
func (qs *QueueSetImpl) enqueue(packet *Packet) {
...
func (qs *QueueSetImpl) Enqueue(packet *Packet) bool {
	qs.LockAndSyncTime()
	defer qs.lock.Unlock()
...
func (qs *QueueSetImpl) updateQueueVirStartTime(packet *Packet, queue *Queue) {
...
func (qs *QueueSetImpl) FinishPacketAndDequeueWithChannelAsMuchAsPossible(pkt *Packet) {
	qs.LockAndSyncTime()
	defer qs.lock.Unlock()
...
// QueueSetImpl
func (qs *QueueSetImpl) finishPacket(p *Packet) {
...
func (qs *QueueSetImpl) dequeue() (*Packet, bool) {
...
func (qs *QueueSetImpl) dequeueWithChannel() (*Packet, bool) {
	pkt, ok := qs.dequeue()
	if !ok {
		return nil, false
	}
