...
func createRequestManagementServerAndClient(
	t *testing.T, delegate http.Handler, flowSchemas []*rmv1a1.FlowSchema,
	priorityLevelConfigurations []*rmv1a1.PriorityLevelConfiguration,
	...
	reqMgmt := utilflowcontrol.NewRequestManagementSystemWithPreservation(
	...
	// TODO(aaron-prindle) flowschemas was always len(0) with the below
	// (even creating flowschemas through client)
...
func generateFlowSchemas(n int) []*rmv1a1.FlowSchema {
	flowSchemas := []*rmv1a1.FlowSchema{}
	for i := 0; i < n; i++ {
		fs := fs(
			fmt.Sprintf("fs-%d", i),
			fmt.Sprintf("plc-%d", i),
		...
		flowSchemas = append(flowSchemas, fs)
	}
	return flowSchemas
}

func generatePriorityLevelConfigurations(n int) []*rmv1a1.PriorityLevelConfiguration {
	priorityLevelConfigurations := []*rmv1a1.PriorityLevelConfiguration{}
	for i := 0; i < n; i++ {
		pl := pl(
			fmt.Sprintf("plc-%d", i),
		...
		priorityLevelConfigurations = append(priorityLevelConfigurations, pl)

		// 	&rmv1a1.PriorityLevelConfiguration{
		// 		ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("plc-%d", i)},
		// 		Spec: rmv1a1.PriorityLevelConfigurationSpec{
...
func TestReqMgmtMultiple(t *testing.T) {
	...
	delegate := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for i := 0; i < countnum; i++ {
			if r.Header.Get("Username") == strconv.Itoa(i) {
	...
	for i := 0; i < countnum; i++ {
		header := make(http.Header)
		header.Add("Username", strconv.Itoa(i))
		req, _ := http.NewRequest("GET", server.URL, nil)
		req.Header = header
	...
	for i, count := range counts {
		if count == 0 {
			t.Errorf("Expected all groups to to serviced, found count with 0 served")
		}
