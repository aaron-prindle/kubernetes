...
func (s *Scheme) Validate(opts *operation.ValidationOpts, object any, subresources ...string) field.ErrorList {
	...
	if fn, ok := s.validationFuncs[reflect.TypeOf(object)]; ok {
		return fn(operation.Context{Operation: operation.Create, Opts: opts}, object, nil, subresources...)
	}
	return nil
}

// ValidateUpdate validates an update to an object using the registered validation function.
func (s *Scheme) ValidateUpdate(opts *operation.ValidationOpts, object, oldObject any, subresources ...string) field.ErrorList {
	if len(s.registrationErrors) > 0 {
		return s.registrationErrors // short circuit with registration errors if any are present
	}
	if fn, ok := s.validationFuncs[reflect.TypeOf(object)]; ok {
...
func (v *ValidationTester) Opts(opts *operation.ValidationOpts) *ValidationTester {
	v.opts = opts
	return v
}
