...
func (istv ifSpecifiedTagValidator) GetValidations(context Context, args []string, payload string) (Validations, error) {
	...
	// Create get field functions for condition field and target field
	// These functions will be used to extract field values in the validation function
	nilableStructType := context.Type
	if !isNilableType(nilableStructType) {
		nilableStructType = types.PointerTo(nilableStructType)
	}

	// Get field function for condition field
	getCondFieldFn := FunctionLiteral{
		Parameters: []ParamResult{{"o", nilableStructType}},
		Results:    []ParamResult{{"", condMemb.Type}},
	}
	// Generate the body for the condition field getter function
	// if isNilableType(condMemb.Type) {
	// 	getCondFieldFn.Body = fmt.Sprintf("return o.%s", condMemb.Name)
	// } else {
	// 	getCondFieldFn.Body = fmt.Sprintf("return &o.%s", condMemb.Name)
	// }
	getCondFieldFn.Body = fmt.Sprintf("return o.%s", condMemb.Name)

	// Get field function for target field
	getTargetFieldFn := FunctionLiteral{
		Parameters: []ParamResult{{"o", nilableStructType}},
		Results:    []ParamResult{{"", targetMemb.Type}},
	}
	// Generate the body for the target field getter function
	// if isNilableType(targetMemb.Type) {
	// 	getTargetFieldFn.Body = fmt.Sprintf("return o.%s", targetMemb.Name)
	// } else {
	// 	getTargetFieldFn.Body = fmt.Sprintf("return &o.%s", targetMemb.Name)
	// }
	getTargetFieldFn.Body = fmt.Sprintf("return o.%s", targetMemb.Name)
	...
	for _, vfn := range validations.Functions {
		...
		f := Function(ifSpecifiedTagName, DefaultFlags, validateIfSpecified,
			// f := Function(ifSpecifiedTagName, vfn.Flags(), validateIfSpecified,
			conditionField, targetField, getCondFieldFn, getTargetFieldFn, WrapperFunction{vfn, targetMemb.Type})
