...
*   **Rollout Plan:** Encapsulate `FieldsV1` with accessors in v1.36 and use build tags to safely opt-in. Flip the default to the `string` implementation in v1.37, and remove the legacy `[]byte` implementation in v1.38.
*   **Memory Savings:** In a 50k minimal pod test, `FieldsV1` allocations dropped by **~80%** (130MB to 27MB), reducing the total `kube-apiserver` heap memory by **~5%** (~1.45 GB down to ~1.37 GB). Based on internal analysis of production clusters with complex pods, we estimate this optimization could yield a **10-25% reduction in total API Server memory usage for large clusters**.
