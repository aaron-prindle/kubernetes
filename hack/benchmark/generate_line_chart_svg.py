...
def create_line_chart(filename, title, pods_data, baseline_vals, optimized_vals, ylabel):
    ...
    for pods in pods_data:
        ...
        svg += f'<text x="{x}" y="{svg_height - margin_bottom + 20}" text-anchor="middle" font-size="12" fill="#666">{pods:,}</text>\n'
    ...
    for i, (pods, val) in enumerate(zip(pods_data, optimized_vals)):
        ...
        if val > 0:
            y_offset = 20 if pods < 100000 else 25 # Ensure labels don't overlap baseline
...
# Data points: [50, 50000, 100000]
pods = [50, 50000, 100000]
# Master Total Heap
baseline = [1200, 1450, 1780.52]  # Approximating base 50 pod cluster heap at ~1.2GB
# Optimized Total Heap
optimized = [1200, 1370, 1613.24] # Approximating base 50 pod cluster heap at ~1.2GB
