#!/usr/bin/env bash
# run-kind-write-contention-benchmark.sh
# End-to-end contention benchmark focusing on parallel WRITES to trigger decoding.
...
MUTEX_PROFILE="$PROFILES_DIR/write-mutex-${BRANCH_NAME//\//-}-${COMMIT_HASH}.prof"
CPU_PROFILE="$PROFILES_DIR/write-cpu-${BRANCH_NAME//\//-}-${COMMIT_HASH}.prof"

echo "=========================================================="
echo " Starting End-to-End API Server WRITE Contention Benchmark"
echo " Branch: $BRANCH_NAME"
echo " Commit: $COMMIT_HASH"
echo " Target Load: $REPLICAS parallel SSA writes"
echo "=========================================================="
...
# We don't need Kwok here since we are just bombarding the API server with ConfigMap SSA writes to trigger Protobuf/JSON decodes.

echo "=> 4. Initiating parallel WRITE requests and capturing profiles..."
...
# Using SSA on ConfigMaps to generate heavy managedFields decode load
timeout 32s bash -c "
  for i in \$(seq 1 $CONCURRENCY); do
    while true; do
      RAND=\$RANDOM
      cat <<JSON | curl -m 5 -s -X PATCH -H 'Content-Type: application/apply-patch+yaml' http://localhost:8001/api/v1/namespaces/default/configmaps/cm-\$i-\$RAND?fieldManager=bench-\$i > /dev/null || true
apiVersion: v1
kind: ConfigMap
metadata:
  name: cm-\$i-\$RAND
