...
func TestDeclarativeValidateForDeclarative(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"invalid RollingUpdate strategy missing rollingUpdate": {
			input: mkDeployment(apps.RollingUpdateDeploymentStrategyType, nil),
			expectedErrs: field.ErrorList{
				field.Required(field.NewPath("spec", "strategy", "rollingUpdate"), "this should be defaulted and never be nil"),
			},
		},
		"invalid Recreate strategy with rollingUpdate": {
			input: mkDeployment(apps.RecreateDeploymentStrategyType, &apps.RollingUpdateDeployment{
				MaxUnavailable: intstr.FromInt32(1),
				MaxSurge:       intstr.FromInt32(1),
			}),
			expectedErrs: field.ErrorList{
				field.Forbidden(field.NewPath("spec", "strategy", "rollingUpdate"), "may not be specified when strategy `type` is 'Recreate'"),
			},
		},
	}
...
func TestDeclarativeValidateUpdateForDeclarative(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"valid update Recreate to RollingUpdate": {
			oldObj: mkDeployment(apps.RecreateDeploymentStrategyType, nil),
			updateObj: mkDeployment(apps.RollingUpdateDeploymentStrategyType, &apps.RollingUpdateDeployment{
				MaxUnavailable: intstr.FromInt32(1),
				MaxSurge:       intstr.FromInt32(1),
			}),
		},
		"invalid update Recreate with rollingUpdate": {
			oldObj: mkDeployment(apps.RecreateDeploymentStrategyType, nil),
			updateObj: mkDeployment(apps.RecreateDeploymentStrategyType, &apps.RollingUpdateDeployment{
				MaxUnavailable: intstr.FromInt32(1),
				MaxSurge:       intstr.FromInt32(1),
			}),
			expectedErrs: field.ErrorList{
		...
		"invalid update RollingUpdate missing rollingUpdate": {
			oldObj:    mkDeployment(apps.RecreateDeploymentStrategyType, nil),
			updateObj: mkDeployment(apps.RollingUpdateDeploymentStrategyType, nil),
			expectedErrs: field.ErrorList{
				field.Required(field.NewPath("spec", "strategy", "rollingUpdate"), "this should be defaulted and never be nil"),
			},
		},
